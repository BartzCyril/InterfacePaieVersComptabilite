export class Month {
    ...
    */
    static getInformationsMonth(year, firstFileLine) {
        if (Month.getMonth(firstFileLine)) {
            const month = Month.getMonth(firstFileLine);
            let informationsMonth;
            switch (month) {
                case "Janvier":
                    informationsMonth = { numberMonth: "01", endOfMonth: "30" };
                    break;
                case "Février":
                    const transformYear = parseInt(year);
